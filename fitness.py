...
def addressSpecificInputType(tokens, parsedTasks, enabledTasks, waitingTokens, inputIndexes, topInputIndexes, bottomInputIndexes, properlyParsedTasks, missingLocalTokens, availableToken, i, l, centralInputGatewayType, topInputGatewayType, bottomInputGatewayType, completenessAttemptFactor1, completenessAttemptFactor2, incorrectlyFiredTasks):
    ...
    if availableToken == 0:
        if incorrectlyFiredTasks.count(l) == 0:
            incorrectlyFiredTasks.append(initPop.getTaskID(initPop.log[i][l]))
        if centralInputGatewayType == 0:
...
def calculateCompletenessAndPreciseness(cromossome, completenessAttemptFactor1, completenessAttemptFactor2):
    ...
    precisenessAKM = len(enabledTasks) / len(initPop.alphabet)
...
def calculateSimplicity(cromossome):
    enabledOuputTasks = 0
...
def evaluateIndividual(cromossome, referenceCromossome, TPweight, precisenessWeight, simplicityWeight, completenessWeight, i, completenessAttemptFactor1, completenessAttemptFactor2):
    TP = calculateTP(cromossome, referenceCromossome)
    completeness = 0
    precisenessAKM = 0
    precisenessBVB = 0
    temporaryAdaptedCromossome = adaptCromossome(copy.deepcopy(cromossome))
    (completeness, precisenessAKM, precisenessBVB, incorrectlyFiredTasks) = calculateCompletenessAndPreciseness(temporaryAdaptedCromossome, completenessAttemptFactor1, completenessAttemptFactor2)
    simplicity = calculateSimplicity(cromossome)
...
def evaluationPopulation(population, referenceCromossome, TPweight, precisenessWeight, simplicityWeight, completenessWeight, completenessAttemptFactor1, completenessAttemptFactor2, selectionOp):
    evaluationSum = 0
    evaluationValues = []
    for i in range(len(population)):
        evaluationValues.append(evaluateIndividual(population[i], referenceCromossome, TPweight, precisenessWeight, simplicityWeight, completenessWeight, i, completenessAttemptFactor1, completenessAttemptFactor2))
